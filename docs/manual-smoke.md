...
11. The Settings tab opens as a hub of section entries (cycle, custom symptoms, tracking, reminders, interface, export/restore, danger zone) plus the `Backup & sync` summary and privacy cards; each entry opens its own section screen with a back action to the hub. The `Cycle Parameters` screen saves cycle parameters locally, the dashboard snapshot reflects the new values after returning, and the `Reminders` screen schedules device-only reminders (toggles, time, lead days) without an account, with clear permission feedback.
...
31. Pregnancy module, locked: with the module not unlocked, a positive pregnancy test surfaces the dashboard entry card in its locked form — an informational card that names the one-time unlock and never routes to plan options or a subscription screen. Everything already logged stays readable.
32. Pregnancy module, unlocked: starting pregnancy tracking walks basis -> date -> confirm with a live due-date and week+day preview, and the dashboard then shows pregnancy mode (week hero, baby-this-week, milestones, red flags, disclaimer) in place of the cycle predictions. Calendar and stats stop painting predictions for as long as the record is active.
33. Kick counter (from week 28) counts taps against the ten-movement target, keeps the elapsed timer running while counting, and files a finished session into its history; discarding asks for confirmation first and leaves no record.
34. Contraction timer starts and stops a contraction from one control, lists completed contractions with duration and interval, and its window summary highlights the 5-1-1 education line only when the pattern actually matches. At term the birth hand-off appears.
35. Ending a pregnancy asks for an explicit confirmation whose dismissal keeps tracking; a birth ending offers postpartum tracking once (only when the module is unlocked), and a loss or other ending goes straight back to the dashboard with neutral copy and no offer.
36. Postpartum mode shows recovery and lochia guidance for the current phase, and the mood check-in offer appears on its own cadence. Completing the EPDS check-in shows the result immediately; answering the self-harm item non-zero adds the crisis-support card with the personal contact and an inline edit, which stays visible with no plan or account.
37. Deleting pregnancy data, postpartum data, and check-in data are three separate actions, each behind device authentication plus a confirm, and each leaves the other two classes and all day-log data intact.
...
2. Manual browser check:
   ...
   - the settings reminders screen renders without runtime errors, stays device-local, keeps the reminder toggles, time, and lead-days controls usable without any account, and offers nothing to buy — reminders are free, so no premium lock card appears at any entitlement state
