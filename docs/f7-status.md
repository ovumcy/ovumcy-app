...
> Update: items 1 and 3 have since landed — `android/` native code was
> generated by `expo prebuild` (present locally; gitignored) and `eas.json`
> is now committed. Items 2 and 4 (the SSL pinning library and real managed
> pin constants) remain outstanding.
