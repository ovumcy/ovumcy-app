...
- **The pregnancy / postpartum / screening module in full.** Pregnancy mode (start wizard, dashboard, milestones, baby-this-week, kick counter, contraction timer with the 5-1-1 window summary, end-of-pregnancy flow incl. clinician re-dating), postpartum mode (recovery matrix, lochia and LAM education, cycle-return offer), and the EPDS mood check-in with its crisis-support surface exist **only** in the app. The canonical web product has no counterpart to any of them: no route, no template, no stored collection — so there is nothing to diff against and nothing to port back. Two review rules follow. First, do not record any part of this module as a parity gap; the checklist's premise is that web is a **floor for Free**, and this module sits outside that floor. Second, keep it distinct from the free-tier **pregnancy test** day-log field above, which *is* web-paritied (same CSV column order, same prediction-pause behaviour): a positive test pauses predictions for every owner, while pregnancy *mode* is the separate one-time-unlock module. The two share a domain word and nothing else — conflating them would either paywall a free field or claim web parity for a surface web does not have.
