...
- Sync transport uses standard TLS with the platform CA chain; it does **not** currently pin server certificates. So health-payload confidentiality against a CA-compromise adversary (one who can obtain a fraudulent but chain-valid certificate) rests on the end-to-end payload encryption above, not on transport pinning — while connection metadata and auth tokens would be exposed to such an adversary. The rationale and revisit criteria are in [docs/sync-trust-model.md](docs/sync-trust-model.md#tls-pinning-posture).
