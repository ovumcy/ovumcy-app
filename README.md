...
- Transport uses standard TLS with platform CA-chain trust; certificate pinning is scaffolded but not yet wired. Payload confidentiality against a CA-compromise adversary (one able to obtain a fraudulent but chain-valid certificate) therefore relies on the end-to-end payload encryption above, not on pinning — while connection metadata and auth tokens would be exposed to such an adversary. The pinning strategy is decided: the managed endpoint will pin the Let's Encrypt ISRG root keys rather than the leaf, so the pin survives certificate renewal. Pin set, rotation rule, and the pre-release checklist that must run before pinning is enabled: [docs/sync-trust-model.md](docs/sync-trust-model.md#tls-pinning-posture).
