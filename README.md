...
- Transport uses standard TLS with platform CA-chain trust; certificate pinning is scaffolded but not yet wired. Payload confidentiality against a CA-compromise adversary therefore relies on the end-to-end payload encryption above, not on pinning (see [docs/sync-trust-model.md](docs/sync-trust-model.md)).
